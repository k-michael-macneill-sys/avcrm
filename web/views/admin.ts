import type { Branch, PublicUser } from '../../src/types/models.js';
import * as api from '../api.js';
import { pageHeader } from '../components.js';
import { empty, h, section, table } from '../dom.js';
 ...
 * endpoint that mints accounts has no business facing the internet on a system
 * holding customers' names and addresses. Somebody's account exists before
 * they arrive, created here by the person who hired them.
 */
 ...
 * to a branch because that is whose crew they run. The distinction is worth
 * spelling out here rather than making somebody infer it from a role dropdown
 * with two entries.
 */
...
export async function renderAdmin(root: HTMLElement): Promise<void> {
  const [branches, users] = await Promise.all([
    ...
    api.get<PublicUser[]>('/users', { page_size: '100' }),
  ]);
  ...
  root.append(
    pageHeader('Company', 'Branches, and the people who work in them'),
    error,
    branchCard(branches, run),
    peopleCard(branches, users, run),
  );
}

function branchCard(
  branches: Branch[],
  run: ReturnType<typeof submitter>,
): HTMLElement {
  const existing = table<Branch>(
    [
  ...
  return section('Branches', existing, h('h3', {}, 'Add a branch'), form.node, add);
}
...
function peopleCard(
  ...
  const existing =
    branches.length === 0
      ? empty('Add a branch first — an operator has to belong to one.')
      : table<PublicUser>(
          [
  ...
  if (branches.length === 0) return section('People', existing);
  ...
  const add = run(
    ...
    async () => {
      ...
      // Corporate sees every branch, so tying the account to one would only
      // narrow it. A manager keeps their branch: it is whose crew they run,
      // and PATCH /branches checks the manager belongs to the branch.
  ...
  return h(
    'div',
    {},
    section('People', existing),
    section('Add someone', form.node, add),
  );
}
